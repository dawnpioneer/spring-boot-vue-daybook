...
import com.nagino.daybook.payload.response.MessageResponse;
import com.nagino.daybook.repository.RoleRepository;
import com.nagino.daybook.repository.UserRepository;
import com.nagino.daybook.security.jwt.JwtUtils;
		...
		if (strRoles == null) {
			...
			strRoles.forEach(role -> {
				...
				case "admin":
					...
					break;
				case "mod":
					Role modRole = roleRepository.findByAuthority(ERole.ROLE_MODERATOR)
							.orElseThrow(() -> new RuntimeException("Error: Role is not found."));
					roles.add(modRole);
