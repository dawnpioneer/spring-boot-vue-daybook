      ...
      <q-dialog v-model="dataForm.isOpenDataFormDialog">
        <q-card class="q-pa-lg" style="max-width: 500px width: 100%">
          ...
          <div class="row q-col-gutter-md">
            <div class="col-12">
              <q-select outlined v-model="dataForm.model.category" :options="options" label="收支" />
            </div>
            <div class="col-12">
            ...
            <div class="col-12">
              <q-input outlined label="排序" v-model="dataForm.model.sortNum"
                lazy-rules
                type="number"
                :rules="[ val => val && val > 0 || '請輸入排序']"
              />
            </div>
            <div class="col-12">
  ...
  methods: {
    ...
    saveData () {
      ...
      // 修改
        ...
        .then(response => {
          let object = this.daybookCategories.filter(item => { return item.id === this.dataForm.data.id })[0]
          if (object.lastUpdated !== response.data.lastUpdated) {
